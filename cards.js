...
var cards = (function() {
  ...
  function init(options) {
    ...
    //modify this to change the order of the cards
    //for (let l = 0; l < opt.loop; l++)
    //alert("end"+end)13
    end=13
    for (let l = 0; l < opt.loop; l++)
      for (var i = start; i <= end; i++) {
        if(i<8+1||i>10) {//restrict it only to 1 to 8 and thend KQJ
          all.push(new Card('c', i, opt.table));//cllubs
          all.push(new Card('s', i, opt.table));//spades
          all.push(new Card('d', i, opt.table));//diamonds
          all.push(new Card('h', i, opt.table));//hearts
        }
      }
    if (opt.blackJoker) {
  ...
  function shuffle(deck) {
    ...
    // make it so that it only shuffles on one side, 
    //and sends the all object form one client to the other if it is the other player
    //this is for the server init
    /// with current configuration actually means it wont shuffle at all
    if(server==true){
      var i = deck.length;
      if (i == 0) return;
      while (--i) {
      ...
      var jdeck =JSON.stringify(deck)
      alert("shuffled")
      socket.emit('send deck', jdeck);
    }
  }
